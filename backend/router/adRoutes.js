...
router.post('/postAd', upload.array('images', 3), async (req, res) => {
  if (!req.user) {
    return res.status(401).json({ message: 'You must be logged in to post an ad.' });
  }

  try {
    const { city, carInfo, registeredIn, exteriorColor, mileage, price, adDescription, mobileNumber } = req.body;
